# [Company Name]

## Identity
- **ISIN:**
- **Ticker:**
- **Country:**
- **Currency:**
- **Sector:**
- **Status:**
- **Thesis health:**
- **Position action:**
- **Fresh capital action:**
- **Last reviewed:**

## 1. Investment thesis

What would have to be true for this company to become an exceptional long-term compounder?

Clearly label **FACT**, **MANAGEMENT CLAIM**, **INFERENCE**, and **SPECULATION** where useful.

## 2. TAM / runway

- Current market opportunity
- Long-term expansion potential
- Multiple growth avenues
- Why the opportunity can persist for 10–25+ years

## 3. Current business / financials

Summarise revenue, margins, profit, cash flow, balance sheet, ROCE/ROIC and other sector-specific metrics. For financial companies use ROE/ROA/PB, asset quality, NIM, capital adequacy and liability franchise instead of industrial-company metrics.

## 4. Moat

Assess technology/IP, cost advantage, switching costs, network effects, distribution, qualification/regulatory barriers, customer relationships and scale advantages.

## 5. Management / capital allocation

Assess promoter/management quality, ownership, incentives, governance, capital allocation history, reinvestment runway and dilution risk.

## 6. Market-cap mathematics

Start with current market cap and calculate:

- 10X
- 50X
- 100X

Reverse-engineer the 100X outcome into required future revenue, margins, profit, market share and CAGR.

## 7. Valuation / expectations priced in

Assess P/E, EV/EBITDA, P/S, P/B, FCF yield, market-cap/revenue, market-cap/profit and/or reverse DCF as appropriate.

Answer explicitly: **How much of the future is already reflected in today's price?**

## 8. Bear / Base / Bull / 100X scenarios

### Bear

### Base

### Bull

### Extreme 100X

State the assumptions behind each scenario. Avoid arbitrary price targets.

## 9. Key risks / why it may NOT become 100X

- Business / competitive risk
- Financial / balance-sheet risk
- Governance / promoter risk
- Valuation risk
- Regulatory / technology risk
- Execution risk
- Customer concentration
- Dilution risk

## Decision-Grade Evidence

Include only evidence that materially affects the thesis, classification, scores, valuation conclusion or a key assumption. Do not cite every routine statement.

### DGE-1 — [Short title]
- **Claim:**
- **Why it matters:**
- **Source:**
- **Date / reporting period:**
- **Location:**
- **Type:** FACT / MANAGEMENT CLAIM / INFERENCE / SPECULATION
- **Basis:** *(required for INFERENCE; identify supporting evidence/DGE items)*

Add further DGE items as needed. The higher the conviction or importance of the decision, the stronger the evidence trail should be.

## 10. Disconfirming evidence

List the evidence that would weaken or invalidate the thesis. Define what should be monitored.

## 11. What must be true for 100X

List the critical conditions required for the extreme outcome and assess how realistic each is.

## 12. Sector / discovery-bias check

- Is the sector attractive, or is the company exceptional?
- Would it qualify if the sector were not fashionable?
- What is the strongest argument against the sector thesis?
- Is it better than a promising company from a different sector?
- Is our universe becoming overrepresented in this theme?

## 13. Verdict

**[STATUS] — Thesis: [THESIS_HEALTH] | Existing position: [POSITION_ACTION] | Fresh capital: [FRESH_CAPITAL_ACTION]**

Explain why.

## 14. 100X scores

| Dimension | Score |
|---|---:|
| Business Potential | /10 |
| Probability of Success | /10 |
| Investment Attractiveness | /10 |
| Overall 100X Score | /10 |

## Beyond the 100X Framework *(only when `beyond_100x: true`)*

Include this section only when research has identified a compelling investment case worth surfacing outside the strict 100X framework.

### Why it is here

### Alternative investment thesis

### What must be true

### Key risks / unresolved questions

Explain clearly why the company deserves separate discovery. This is not a second status, score, portfolio, or recommendation. Do not add this section for `beyond_100x: false` companies. CORE companies must not use this section because they are already surfaced by the primary 100X framework.

## Research status

- **Decision-Grade Evidence status:** Not applicable / Partial / Complete for current research scope
- **Primary sources reviewed:**
- **Latest material event:**
- **Next research question:**
- **Last reviewed:**
